...
# integration lane attaches over HTTP. The separate package probe uses the
# default Arrow backend: vgi-rpc's stdio path is Arrow JS, while Flechette is an
# HTTP/Workers backend and cannot validate its schema objects on that path.
if [ -z "${VGI_DATABASE_PACKAGE_WORKER:-}" ]; then
  export VGI_DATABASE_PACKAGE_WORKER="VGI_BUN_CONDITIONS= $WORKER"
fi
...
# ---------------------------------------------------------------------------
# Byte-stream workers must not inherit VGI_BUN_CONDITIONS.
#
# The bin/ wrappers pass $VGI_BUN_CONDITIONS to bun, which is how the flechette
# lane selects that Arrow backend. That works for an HTTP worker and is broken
# by construction for a stdio / AF_UNIX / TCP one: @query-farm/vgi-rpc's
# IpcStreamReader decodes the request stream with arrow-js unconditionally
# (a hard `import { RecordBatchReader } from "@query-farm/apache-arrow"`,
# not the `#vgi-rpc-arrow` backend), so a flechette worker on a byte-stream
# transport compares an arrow-js-decoded request schema against its own
# flechette-built parameter schema and rejects every method with a non-empty
# params schema:
#
#   ProtocolError: Parameter schema mismatch for method 'catalog_attach'
#   field 'request': expected type [object Object], got Binary.
#
# ("[object Object]" is a flechette DataType stringified; "Binary" is arrow-js.)
#
# This is the same boundary the matrix comment in .github/workflows/ci.yml
# describes when it says there is deliberately no launch/flechette lane. The
# http lane still needs the two *subprocess* versioned wrappers (see the http
# case below), so route them through a shim that clears the variable rather
# than dropping five files that have nothing to do with the Arrow backend —
# what they assert (bind-time version resolution) is backend-agnostic, and the
# flechette lane already covers the whole rest of the suite over HTTP.
# ---------------------------------------------------------------------------
SHIM_DIR="$STAGE/shims"
stdio_shim() {
  local exe="$1" shim
  [ -n "${VGI_BUN_CONDITIONS:-}" ] || { printf '%s\n' "$exe"; return 0; }
  mkdir -p "$SHIM_DIR"
  shim="$SHIM_DIR/$(basename "$exe")"
  {
    printf '#!/usr/bin/env bash\n'
    printf '# Generated by ci/run-integration.sh: flechette cannot drive a\n'
    printf '# byte-stream transport, so this worker runs on the default backend.\n'
    printf 'unset VGI_BUN_CONDITIONS\n'
    printf 'exec %s "$@"\n' "$(printf '%q' "$exe")"
  } > "$shim"
  chmod +x "$shim"
  printf '%s\n' "$shim"
...
case "$TRANSPORT" in
  ...
  http)
    ...
    # …via stdio_shim: a byte-stream worker must not inherit
    # VGI_BUN_CONDITIONS (see the stdio_shim comment above). No-op unless the
    # lane sets it, so the arrow-js lane runs the wrappers directly.
    export VGI_VERSIONED_WORKER="$(stdio_shim "$VERSIONED")"
    export VGI_VERSIONED_TABLES_WORKER="$(stdio_shim "$VERSIONED_TABLES")"
