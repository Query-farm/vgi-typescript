...
function makeBindRequest(opts: {
  ...
  return {
    ...
    settings: null,
    secrets: null,
    attach_opaque_data: opts.attach_opaque_data ?? null,
    transaction_opaque_data: opts.transaction_opaque_data ?? null,
    resolved_secrets_provided: false,
  };
}

describe("scalar bind exposes attach_opaque_data / transaction_opaque_data", () => {
  ...
  test("forwards attach_opaque_data when present", () => {
    captured = null;
    const attach = new Uint8Array(16).fill(0xaa);
    fn.bind!(makeBindRequest({ attach_opaque_data: attach }));
    expect(captured).not.toBeNull();
    expect(captured!.bindCall.attach_opaque_data).toEqual(attach);
    expect(captured!.bindCall.transaction_opaque_data).toBeNull();
  });

  test("forwards transaction_opaque_data when present", () => {
    captured = null;
    const attach = new Uint8Array(16).fill(0xaa);
    const tx = new Uint8Array(8).fill(0xbb);
    fn.bind!(makeBindRequest({ attach_opaque_data: attach, transaction_opaque_data: tx }));
    expect(captured!.bindCall.attach_opaque_data).toEqual(attach);
    expect(captured!.bindCall.transaction_opaque_data).toEqual(tx);
  });

  test("both null when invoked outside a catalog", () => {
    captured = null;
    fn.bind!(makeBindRequest({}));
    expect(captured!.bindCall.attach_opaque_data).toBeNull();
    expect(captured!.bindCall.transaction_opaque_data).toBeNull();
  });
});
