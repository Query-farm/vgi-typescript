...
### The landing surface lives here, not in vgi-rpc

`src/http/landing.ts` serves `GET {prefix}/` (the shared `landing.html`, or a JSON
status document for `?format=json`) and `GET {prefix}/vgi-client.js`. Both assets are
vendored into this repo — `src/http/landing-html.ts` and `src/http/client-bundle.ts`,
regenerated by `scripts/vendor-landing-html.mjs` / `scripts/vendor-client-bundle.mjs`
from `vgi-web-frontend/public/`.

They used to live in `@query-farm/vgi-rpc`, which was backwards: vgi-rpc is generic RPC
over Arrow, depends on nothing here, and was shipping a page that renders VGI catalogs
plus a **compiled bundle of this package's own client** — a build artifact of the higher
layer inside the lower one, and 911 KB of vgi-rpc's 1269 KB bundle. vgi-python always
drew the line this way (`vgi/http/` has the assets, `vgi_rpc/http/` has neither).

The surface reaches the request through vgi-rpc's generic `extraRoutes` hook, not by
wrapping the returned handler. Ordering is the reason: contributed routes run after the
OAuth browser-redirect branch, so a worker with PKCE configured still sends an
unauthenticated browser to its identity provider first. A wrapper outside the handler
would answer ahead of that redirect. Note the flip side — vgi-rpc gates GET pages on
`authenticate && pkceConfig`, and a non-browser that fails auth falls through to page
serving, so **on a worker without PKCE the page and the client bundle are public**.
